    ...
    while (1) {
        ...
        if (p != 0 && q != 0 && p != q) {
            ...
            *n = p * q;
            ...
            if ((*n >= 9223372036854775808UL) && (*n <= 18446744073709551615UL)){ //
                break;
            }
...
int mRSA_cipher(uint64_t *m, uint64_t k, uint64_t n)
{
    ...
    int result = 1;
    // 평문, 암호문 모두 n의 크기를 넘을 수 없습니다.
    // 넘는 경우 1을 리턴합니다.
    if (*m >= n || *m < 0) {
        return result;
    }
    // 암호화일 때는 공개키를, 복호화일 때는 개인키를 활용하여
    // m^k mod n을 구합니다(k는 공개키 또는 개인키).
    // 인증 시에는 송신자가 인증수단을 개인키로 암호화하고
    // 수신자가 공개키로 복호화합니다.
    *m = mod_pow(*m, k, n);
